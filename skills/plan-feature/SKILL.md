...
Invoke the `grill-with-docs` skill. Let it run to completion — this is an interactive session that sharpens the plan against the existing domain model and updates `CONTEXT.md` and `docs/adr/` inline as decisions crystallise. Do not move to step 2 until the session concludes and the user is happy with the outcome.
...
After the issues are created, check for uncommitted changes to `CONTEXT.md` and `docs/adr/`:
...
git commit -m "docs: update CONTEXT.md and ADRs after planning session"
git push
```
